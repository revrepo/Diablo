...
var logLevel = "info";
if(settings.winston_syslog_level && settings.winston_syslog_level!=undefined){
	logLevel = settings.winston_syslog_level;
}
...
/**
function for printing debug msgs
*/
var log_debugMsg = function(msg,val){
	if(logLevel=="debug"){
		delogger.log("debug",msg,val);
	}
}
/**
*Md5 key generation
...
var authkey = function(){
	log_debugMsg("MD5 authentication key generation");
...
function openCubeConnection(){
	if(settings.is_multiple_collectors){
		log_debugMsg("Open socket connection for multiple cubes");
		...
		log_debugMsg("Open socket connection for Single cube");
...
log_debugMsg("Validating Navigation Timing params");
        var flag = true;
        if(params.hasOwnProperty("nt_dns_end") &&
			...
			log_debugMsg("Navigation Timing : Valid Params");
                flag = true;
        }else{
			logger.log("alert","Navigation Timing : Missing mandatory Params");
                return false;
        }
        Object.keys(params).forEach(function(key){
                ...
                if(key=="nt_dns_end" ||
                        ...
                        if(isNaN(parseInt(value))){
							logger.log("alert","Navigation Timing : Invalid '"+params[key]+"'Value");
                                flag = false;
                                return;
                        }
                }
...
function validate_nt_info_Request(params){
	log_debugMsg("Validating Roundtrip Params");
	var flag = true;
	if(params.hasOwnProperty("t_done")){
		if(params.hasOwnProperty("t_resp")){
			log_debugMsg("Roundtrip : Valid Params");
	                flag = true;
		}else{
			log_debugMsg("Roundtrip : Missing Params");
			return false;
		}
        }else{
			log_debugMsg("Roundtrip : Missing Params");
	...
	Object.keys(params).forEach(function(key){
                ...
                if(key=="t_resp" || key =="t_done" ){
                        if(isNaN(parseInt(value))){
							logger.log("alert","Roundtrip : Invalid '"+params[key]+"' value");
                                flag = false;
                                return;
                        }
                }
...
var deviceType =  function(value){
	log_debugMsg("Validating the Device type");
...
var cubeSend = function(object,client){
	...
	if(object.data.user_ip && object.data.user_ip != undefined ){
		log_debugMsg("Fetching Location info from User IP");
...
log_debugMsg("obj----to cube-->",object);
logger.log("info","Sending Object to Cube");
	client.send(object);
}
process.on("uncaughtException", function (error) {
logger.log("emerg","ERROR HANDLING-->",error.stack);
...
route.get("/service", function(request, response) {
	...
	logger.log("info","Received request with id - "+transactionId);
	...
	log_debugMsg("USER IP-->",user_ip);

	log_debugMsg("Parsing the request");	
	var url_parts = url.parse(request.url, true);
	log_debugMsg("Fetching query string");
	...
	if(settings.process_res_timing && settings.process_res_timing != undefined){
		 log_debugMsg("Processing third party resources");
		...
		log_debugMsg("Final third party resource JSON-->",finalThirdParty);
		}
	}
	...
	if (settings.service.nonce){
		if(query.hasOwnProperty("nonce")){
			...
			if(query.nonce === md5(authkey())){
				...
				logger.log("alert","Invalid nonce value");
				nonce_value = false;
			}
		}else{
			nonce_value = false;
...
//        console.log("Came in to the HTTP");
        ...
        logger.log("info","BEACON URL LISTENING ON http://"+settings.service.url+":"+settings.service.http_port);
}
...
if (cluster.isMaster) {
  ...
  cluster.on("exit", function(worker, code, signal) {
	if( signal ) {
        logger.log("error", "worker was killed by signal: "+signal);
      } else if( code !== 0 ) {
        logger.log("error","worker exited with error code: "+code);
      }/*else{
